...
在最新版中，我们引入了多项高质感微动效以及灵感整理合成功能：
- **🎲 卡牌 3D 翻转动效**：当点击“换一批”词汇时，未锁定的词卡将触发精致的 Y 轴 3D 翻转过渡动效。在翻转到 90 度侧面时静默完成文本替换并瞬移偏角，完美规避卡片背面文字镜像的 Bug，流畅度与视觉效果极佳。
- **🎁 每日限定灵感盲盒**：首页 Bento 顶部追加了每日限定一次的通栏“今日灵感盲盒”。未开盒时带彩虹发光与趣味抖动动效（`animate-jiggle`），开盒时播放礼包开箱动效，抽取今日随机派发的 **3词条 + 1场景/挑战题目**，并支持一键以此开启写作并载入对应写作模式。
- **🧩 灵感片段多选合成器**：
  - **精美炫彩选中环**：在“灵感宫殿”中提供多选模式，选中的卡片将带起绚丽的旋转彩色微光环绕边框（`palace-selected-ring`）。
  - **片段排序与拼接预览**：支持大弹窗两栏拼图面板，允许使用 `▲` / `▼` 对所勾选片段进行直观的先后顺序调整，并可修改标题及实时预览拼接后的 Markdown 长文。
  - **新草稿一键生成入库**：点击确认后，自动计算字数并合成为一篇全新的 `free` 自由写作草稿，写入 Dexie 数据库，自动加载到编辑器并重定向。
- **⚙️ 无感多选切换**：优化了选择模式的可见性逻辑，多选功能在项目 $\ge 2$ 时默认可用，不再受 AI 开启状态的强绑定。
...
- **灵感宫殿与合成器**：以精美卡片陈列历史草稿（选中带旋转炫彩环），支持多选、上下移动排序、自动拼接预览并一键合成新长文；同时支持写作模式过滤与关键词搜索。
