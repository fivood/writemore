...
### Changed
- UI: 灵感宫殿卡片重设计——每种写作模式卡片加入左侧 4px 彩色边框锚点、顶部模式主色调渐变底色、右下角极低透明度水印图标、hover 时更明显上浮和投影；词条 chip 改为对应模式配色。

### Fixed
- 旧稿件模式分类错误：v1 数据库中无 `writingMode` 字段的草稿（自由创作、梦境等）全部被误判为「词汇灵感」；现在打开灵感宫殿时会根据关联词条是否有实际词语自动推断并写回数据库，一次性静默修复历史数据。

## [0.1.9] - 2026-04-10

### Added

- **应用内自动更新**：桌面端集成 `tauri-plugin-updater`，启动 3 秒后静默检测 GitHub 最新版本；发现新版本时顶部横幅提示，用户点击「立即下载安装」可在应用内完成下载并实时显示进度百分比，下载完成后点击「重启以完成更新」即可；Web 版保留原有跳转 GitHub Release 页的方式。
...
- **多义词扩充**：
  ...
  - 新增「克隆人」（人物/科幻），与 w0009「克隆」（动作）区分
  - 新增「信」（实物/书信），与 w1457「信」（抽象/诚信）区分
